...
const Quiz = () => {
    ...
    return (
        <section className="cont" style={{ display: `${showQuiz ? 'block' : 'none'}` }}>
            <div className="container">
                <div className="row vh-100 align-items-center justify-content-center">
                    <div className="col-lg-8">
                        <div className="back" style={{ background: 'lightgreen', borderColor: '#646464' }}>
                            <div className="d-flex justify-content-between gap-md-3">
                                <h5 className='mb-2 fs-normal lh-base'>{question?.question}</h5>
                                <h5 style={{ color: 'black'}}>{quizs.indexOf(question) + 1} / {quizs?.length}</h5>
                            </div>
                            <div className='btn'>
                                {
...

                            {
                                (questionIndex + 1) !== quizs.length ?
                                    <button className='btn py-2 w-100 mt-3 bg-primary text-light fw-bold' onClick={nextQuestion} disabled={!selectedAnswer} style={{background:'white'}}>Next Question</button>
                                    :
